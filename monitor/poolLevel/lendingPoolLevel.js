...
export const lendingPoolLevelTimeseries = async (deployments) => {
    const endpointsList = [];
    Object.keys(deployments).forEach((depo) => {
        if (
            !deployments[depo].indexingError &&
    ...
    query MyQuery {
      ...
      marketDailySnapshots(first: 100) {
        id
        name
        totalValueLockedUSD,
      ...
      marketHourlySnapshots(first: 100) {
        id
        name
        totalValueLockedUSD,
    ...
    await Promise.all(promiseArr)
        .then(
            ...
            (marketLevelData = response.map((marketData) => {
                return {
                    marketDailySnapshots: marketData?.data?.data?.marketDailySnapshots || [],
                    marketHourlySnapshots: marketData?.data?.data?.marketHourlySnapshots || [],
                    url: marketData.config.url,
                    deployment: Object.keys(deployments).find((key) => deployments[key].url === marketData.config.url) || marketData.config.url.split("messari/")[1],
                };
            }))
    ...
    marketLevelData.forEach((protocol, idx) => {
        console.log('marketLevelData', protocol.deployment, idx)
        if (!protocol?.marketDailySnapshots) return;
        let data = protocol.marketDailySnapshots;
        if (!data?.length) return;
        ...
        if (!protocol?.marketHourlySnapshots) return;
        data = protocol.marketHourlySnapshots;
        if (!data?.length) return;
        url = protocol.url;
